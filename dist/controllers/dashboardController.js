...
const getDashboard = async (req, res) => {
    if (req.session && req.session.user) {
        try {
            const users = await User.find().select('-password -token -__v -_id -isTokenUsed -email -createdAt -updatedAt').lean();
            const characters = await Character.find().select('-desc -vision -weapon -versionRelease -birthday -title -constellation -region -affiliation -model -wikiUrl').lean();
            const characterCount = await Character.countDocuments();
            const weapons = await Weapon.find().select('-versionRelease').lean();
            const weaponCount = await Weapon.countDocuments();
            const artifacts = await Artifact.find().select('-versionRelease').lean();
...
const uploadCharacterFile = async (req, res) => {
    try {
        if (!req.file) {
...
const uploadWeaponFile = async (req, res) => {
    try {
        if (!req.file) {
            req.flash('error', 'no files selected!');
            return res.status(301).redirect('/dashboard');
...
const uploadArtifactFile = async (req, res) => {
    try {
        if (!req.file) {
            req.flash('error', 'no files selected!');
            return res.status(301).redirect('/dashboard');
...
const editArtifact = async (req, res) => {
    const { id } = req.params;
    try {
        if (req.session.user && req.session.role === 'admin') {
...
const saveWeapon = async (req, res) => {
    const { id } = req.params;
...
const downloadCharacters = async (req, res) => {
    try {
        if (req.session.user && req.session.role === 'admin') {
...
const downloadWeapons = async (req, res) => {
    try {
        if (req.session.user && req.session.role === 'admin') {
...
const downloadArtifacts = async (req, res) => {
    try {
        if (req.session.user && req.session.role === 'admin') {
