...
const getSingleArtifact = async (req, res) => {
    ...
    try {
        ...
        if (name) {
            ...
            const formattedInput = formattedName.replace(new RegExp(`\\b(${wordsToExclude.join('|')})\\b`, 'ig'), (match) => match.toLowerCase());
            const getArtifactByName = await Artifact.findOne({ name: formattedInput }).select('-_id -__v -fullSet.sands -fullSet.plume -fullSet.circlet -fullSet.goblet -fullSet.flower.piece -fullSet.flower.title');
            if (infoSize == 'full') {
                const getArtifactByName = await Artifact.findOne({ name: formattedInput }).select('-_id -__v');
...
const getAllArtifacts = async (req, res) => {
    ...
    try {
        ...
        if (infoSize == 'full') {
            const fullArtifactData = await Artifact.find().select('-__v -_id');
            apiLogger.verbose('API call successful', {
        ...
        const minimalArtifactData = await Artifact.find().select('-_id -__v -fullSet.sands -fullSet.plume -fullSet.circlet -fullSet.goblet -fullSet.flower.piece -fullSet.flower.title');
        apiLogger.verbose('API call successful', {
