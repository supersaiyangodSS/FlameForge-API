...
const getSingleCharacter = async (req, res) => {
    ...
    try {
        ...
        if (name) {
            ...
            const getCharacterByName = await Character.findOne({ name: formattedInput }).select('-_id -__v -versionRelease -birthday -title -images.card -images.gacha -wikiUrl -affiliation -constellation');
            if (infoSize == 'full') {
                const getCharacterByName = await Character.findOne({ name: formattedInput }).select('-_id -__v');
...
const getAllCharacters = async (req, res) => {
    ...
    try {
        ...
        if (vision) {
            const visionAsString = vision;
            const formattedVision = visionAsString.charAt(0).toUpperCase() + visionAsString.slice(1);
            const getCharacterByVision = await Character.find({ vision: formattedVision }).select('-_id -__v -versionRelease -birthday -title -images.card -images.gacha -wikiUrl -affiliation -constellation');
            if (infoSize == 'full') {
                apiLogger.verbose('API call successful', {
                ...
                const getCharacterByVision = await Character.find({ vision: formattedVision }).select('-_id -__v');
        ...
        if (region) {
            ...
            if (infoSize == 'full') {
                ...
                const getCharacterByRegion = await Character.find({ region: { $in: [formattedInput] } }).select('-_id -__v');
            ...
            const getCharacterByRegion = await Character.find({ region: { $in: [formattedInput] } }).select('-_id -__v -versionRelease -birthday -title -images.card -images.gacha -wikiUrl -affiliation -constellation');
            return res.send(getCharacterByRegion);
        }
        if (rarity) {
            if (infoSize == 'full') {
                ...
                const getCharacterByRarity = await Character.find({ rarity: rarity }).select('-_id -__v');
            ...
            const getCharacterByRarity = await Character.find({ rarity: rarity }).select('-_id -__v -versionRelease -birthday -title -images.card -images.gacha -wikiUrl -affiliation -constellation');
            return res.send(getCharacterByRarity);
        }
        if (weapon) {
            const weaponAsString = weapon;
            ...
            if (infoSize == 'full') {
                ...
                const getCharacterByWeaponType = await Character.find({ weapon: formattedWeapon }).select('-_id -__v');
            ...
            const getCharacterByWeaponType = await Character.find({ weapon: formattedWeapon }).select('-_id -__v -versionRelease -birthday -title -images.card -images.gacha -wikiUrl -affiliation -constellation');
            return res.send(getCharacterByWeaponType);
        }
        if (infoSize == 'full') {
            apiLogger.verbose('API call successful', {
            ...
            const fullCharacterData = await Character.find().select('-__v -_id');
        ...
        const minimalCharacterData = await Character.find().select('-_id -versionRelease -birthday -title -images.card -images.gacha -wikiUrl -__v -affiliation -constellation');
