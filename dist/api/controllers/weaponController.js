...
const getSingleWeapon = async (req, res) => {
    ...
    try {
        ...
        if (name) {
            ...
            const getWeaponByName = await Weapon.findOne({ name: formattedInput }).select('-_id -__v -images.original -images.awakened -images.gacha -source -baseSubStat -affix -passive -versionRelease -region -wikiUrl');
            if (infoSize == 'full') {
                const getWeaponByName = await Weapon.findOne({ name: formattedInput }).select('-_id -__v');
...
const getAllWeapons = async (req, res) => {
    ...
    try {
        ...
        if (family) {
            const familyAsString = family;
            const formattedFamily = familyAsString.charAt(0).toUpperCase() + familyAsString.slice(1);
            const getWeaponByFamily = await Weapon.find({ family: formattedFamily }).select('-_id -__v -images.original -images.awakened -images.gacha -source -baseSubStat -affix -passive -versionRelease -region -wikiUrl');
            if (infoSize == 'full') {
                apiLogger.verbose('API call successful', {
                ...
                const getWeaponByFamily = await Weapon.find({ family: formattedFamily }).select('-_id -__v');
        ...
        if (rarity) {
            if (infoSize == 'full') {
                ...
                const getWeaponByRarity = await Weapon.find({ rarity }).select('-_id -__v');
            ...
            const getWeaponByRarity = await Weapon.find({ rarity }).select('-_id -__v -images.original -images.awakened -images.gacha -source -baseSubStat -affix -passive -versionRelease -region -wikiUrl');
            return res.send(getWeaponByRarity);
        }
        if (infoSize == 'full') {
            apiLogger.verbose('API call successful', {
            ...
            const fullWeaponData = await Weapon.find().select('-__v -_id');
            return res.json(fullWeaponData);
        }
        apiLogger.verbose('API call successful', {
        ...
        const minimalWeaponData = await Weapon.find().select('-_id -__v -images.original -images.awakened -images.gacha -source -baseSubStat -affix -passive -versionRelease -region -wikiUrl');
